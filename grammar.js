...
module.exports = grammar({
  ...
  supertypes: ($) => [$._expression, $._literal, $._declaration],

  // conflicts: ($) => [
  //   [$._expression, $._lowercase_name],
  // ],
  ...
  rules: {
    program: ($) =>
      seq(repeat(seq($.use, optional(";"))), repeat($._declaration)),

    _declaration: ($) =>
      choice($.function_declaration, $.enum_declaration, $.module_declaration, $.type_alias_declaration),
    ...
    use_path: ($) =>
      prec(
        PREC.field,
        seq(choice($._name, $.use_path), ".", choice($.use_set, $._name)),
      ),
    ...
    parameter: ($) =>
      seq(field("name", $._lowercase_name), ":", field("type", $._type)),
    ...
    call_expression: ($) => prec(PREC.call,
        seq(
          field('function', choice($._lowercase_name, $._uppercase_name)),
          field('arguments', $.arguments)
        )
      ),
    arguments: ($) => seq("(", zero_or_more($._expression), ")"),
    ...
    _foreach_body: ($) => choice(seq("yield", $._expression), $._expression),
    gets: ($) => seq($._lowercase_name, "<-", $._expression),
    filter: ($) => seq("if", $._expression),
    if: ($) =>
      seq("if", "(", $._expression, ")", $._expression, "else", $._expression),
    match: ($) =>
      seq(
        "match",
        $._expression,
    ...
    block: ($) => seq("{", $._stmt, "}"),
    field: ($) => prec(PREC.field, seq(choice($._lowercase_name, $._uppercase_name, $.field), ".", $._name)),
    ref: ($) => seq("ref", $._expression, "@", $._lowercase_name),
    deref: ($) => prec(PREC.deref, seq("deref", $._expression)),
    assign: ($) =>
      prec.left(PREC.assign, seq($._expression, ":=", $._expression)),
    region: ($) =>
      seq(
        "region",
        field("name", $._lowercase_name),
    ...
    let: ($) =>
      seq("let", $._pattern, optional(seq(":", $._type)), "=", $._expression),
    ...
    binary: ($) =>
      choice(
        prec.left(
          ...
          seq(
            $._expression,
            alias($.operator_name, $.user_operator),
            $._expression,
          ),
        ),
        prec.left(PREC.composition, seq($._expression, "<+>", $._expression)),
        prec.left(
          PREC.infix_function,
        ...
        prec.left(PREC.cons, seq($._expression, "::", $._expression)),
        prec.left(
          PREC.append,
    ...
    _type_identifier: ($) =>
      choice(alias($.lowercase_name, $.polymorphic_identifier), $.type),
    type: ($) => seq($._uppercase_name, optional($.type_arguments)),
    ...
    modifier: ($) =>
      choice("pub", "inline", "lawful", "opaque", "override", "sealed"),
